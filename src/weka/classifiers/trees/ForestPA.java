...
public class ForestPA extends RandomizableClassifier implements OptionHandler,
   ...
   * @throws java.lang.Exception if there is a problem loading the test dataset
   */
    ...
    * @return random double value within appropriate range
    */
    public double weightRange(int lambda) {
        ...
        if(lambda == 1) {
            max = Math.exp(-1d/lambda);
            min = 0;
        }
        else {
            max = Math.exp(-1d/lambda);
            min = Math.exp(-1d/(lambda-1));
        }
        returnVal = min + (randomNum.nextDouble() * (max - min));
    ...
    * @param lambda level at which the attribute was selected
    * @param height overall height of the tree
    * @return updated attribute weight
   */
    public double weightIncrementValueUpdate(double weight, int lambda, int height) {
    ...
    * @return level this appears on
    */
    public int levelCount(String haystack, String needle) {
    ...
    */
    ...
    public void buildClassifier(Instances instances) throws Exception {
        ...
        randomNum = new Random(m_Seed);
        
        weightSet = new double[dataSet.numAttributes()];
        weightIncrements = new double[dataSet.numAttributes()];
        
        //set weight array up
        for(int i = 0; i < weightSet.length; i++) {
            weightSet[i] = 1;
            weightIncrements[i] = 0;
        }
        
        trees = new SimpleCartPA[numberOfTrees];
        ...
        for(int i = 0; i < numberOfTrees; i++) {
            ...
            */
            Instances baggedDS = dataSet.resampleWithWeights(randomNum);
            ...
            */
            ...
            for(int j = 0; j < dataSet.numAttributes(); j++) {
                ...
                appearanceLevels[j] = Integer.MAX_VALUE;
                
                for(int k = 0; k < treeArray.length; k++) {
                    
                    if(treeArray[k].contains(thisAttrName)) {
                        int thisLev = levelCount(treeArray[k], "|  ");
                        if(thisLev < appearanceLevels[j]) {
                            appearanceLevels[j] = thisLev + 1;
                        }
                    }
                    
                } // end treearray loop
                
                if(appearanceLevels[j] == Integer.MAX_VALUE) {
                    appearanceLevels[j] = 0;
                }
            ...
            */
            for(int j = 0; j < weightSet.length; j++) {
                ...
                if(appearanceLevels[j] > 0) {
                    weightSet[j] = weightRange(appearanceLevels[j]);
                    weightIncrements[j] = weightIncrementValueUpdate(weightSet[j], 
                        appearanceLevels[j], treeHeight);
                }
                
            } //end weight loops
            
            /* STEP 4
                Update weights of the applicable attributes, using their 
                respective weight increment values
            */
            for(int j  = 0; j < weightSet.length; j++) {
                if(appearanceLevels[j] == 0 && weightSet[j] < 1.0) {
                    weightSet[j] = weightSet[j] + weightIncrements[j];
                }   
            } //end increment loops
    ...
    */
    ...
    public String toString() {
        ...
        if(trees == null) {
            return "No ForestPA Built Yet.";
        }
        
        StringBuilder treeString = new StringBuilder();
        for(int i = 0; i < numberOfTrees; i++) {
            treeString.append("\n\nTree ").append(i).append(":\n");
            treeString.append(trees[i].toString());
        }
    ...
    */
    public String globalInfo() {
        return "Class implementing decision forest algorithm Forest PA, \n"
          + "using bootstrap samples and penalized attributes. "
    ...
    */
    ...
    public void setOptions(String[] options) throws Exception {
        ...
        if (optionString.length() != 0) {
            setNumberOfTrees(Integer.parseInt(optionString));
        }
        else {
            setNumberOfTrees(10);
        }
        ...
        if (optionString.length() != 0) {
            setSimpleCartPruningFolds(Integer.parseInt(optionString));
        }
        else {
            setSimpleCartPruningFolds(2);
        }
        
        optionString = Utils.getOption('M', options);
        if (optionString.length() != 0) {
            setSimpleCartMinimumRecords(Integer.parseInt(optionString));
        }
        else {
            setSimpleCartMinimumRecords(2);
        }
    ...
    */
    ...
    public String[] getOptions() {
        ...
        options.add("-T");
        options.add("" + getNumberOfTrees());
        
        options.add("-M");
        options.add("" + getSimpleCartMinimumRecords());
        
        options.add("-N");
        options.add("" + getSimpleCartPruningFolds());
    ...
    */
    ...
    public double classifyInstance(Instance instance) throws Exception {
        ...
        double returnVal = Utils.maxIndex(classes);
        return returnVal;
    ...
    */
    ...
    public double[] distributionForInstance(Instance instance) throws Exception {
        ...
        for(int i = 0; i < numberOfTrees; i++) {
            ...
            for(int j = 0; j < thisDist.length; j++) {
                classes[j] += thisDist[j];
            }
        }
        
        Utils.normalize(classes);
        return classes;    
    }
    ...
    public TechnicalInformation getTechnicalInformation() {
        ...
        result.setValue(TechnicalInformation.Field.AUTHOR, "Md Nasim Adnan and Md Zahidul Islam");
        result.setValue(TechnicalInformation.Field.TITLE, "Forest PA: Constructing a decision forest by penalizing attributes used in previous trees");
        ...
        result.setValue(TechnicalInformation.Field.VOLUME, "89");
        result.setValue(TechnicalInformation.Field.PAGES, "389-403");
    ...
    */
    ...
    private class SimpleCartPA extends RandomizableClassifier implements
        ...
        public TechnicalInformation getTechnicalInformation() {
          ...
          result
            .setValue(TechnicalInformation.Field.AUTHOR,
        ...
        protected void makeTree(Instances data, int totalInstances,
               ...
               for (int i = 0; i < data.numAttributes(); i++) {
                   giniGains[i] = giniGains[i] * weightSet[i];
               }
               ...
               if (totalWeight < 2 * minNumObj || giniGains[attIndex] == 0
                 ...
                 m_isLeaf = false;
                 m_Successors = new SimpleCartPA[2];
                 for (int i = 0; i < 2; i++) {
                   m_Successors[i] = new SimpleCartPA();
        ...
        public void prune(double alpha) throws Exception {
          ...
          while (prune) {
            ...
            SimpleCartPA nodeToPrune = nodeToPrune(nodeList);
        ...
        protected void unprune() {
          if (m_Successors != null) {
            m_isLeaf = false;
            for (SimpleCartPA m_Successor : m_Successors) {
              m_Successor.unprune();
            }
          }
        }
         ...
         */
        protected SimpleCartPA nodeToPrune(Vector<SimpleCartPA> nodeList) {
          if (nodeList.size() == 0) {
            return null;
          }
          if (nodeList.size() == 1) {
          ...
          SimpleCartPA returnNode = nodeList.elementAt(0);
          double baseAlpha = returnNode.m_Alpha;
          for (int i = 1; i < nodeList.size(); i++) {
            SimpleCartPA node = nodeList.elementAt(i);
       ...
       public String toString() {
         ...
         */
        protected Vector<SimpleCartPA> getInnerNodes() {
          Vector<SimpleCartPA> nodeList = new Vector<SimpleCartPA>();
         ...
         */
        protected void fillInnerNodes(Vector<SimpleCartPA> nodeList) {
          if (!m_isLeaf) {
            nodeList.add(this);
            for (SimpleCartPA m_Successor : m_Successors) {
